...
export const TableEquipos = ({ data }) => {
    const dispatch = useDispatch();
    const handleEditar = (item) => {
        console.log(item)
        dispatch(openCloseModalEquipos(true, item, item.tipo_equipo));
    }
    const [contador, setContador] = useState(0);
    const [miPagina, setmiPagina] = useState(1);
    const { registros, paginas } = useSelector(state => state.equipo)
    ...
    const handlePaginar = (index, pagina) => {
        ...
        if (valor >= 0 && valor <= registros) {
            console.log(valor);
            dispatch(getEquiposAdmin(valor));
