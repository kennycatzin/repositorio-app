...
import { eliminarTipoEquipo, getTiposEquiposAdmin, openCloseModalTipoEquipos } from '../../../../actions/tipoEquipos';
import { TipoEquipoModal } from '../modal/TipoEquipoModal';

const cabeceras = [
    "Tipo de equipo",
    "Descripción"
];
export const TableTiposEquipos = ({data}) => {
    const dispatch = useDispatch();
    const handleEditar = (item) => {
       dispatch(openCloseModalTipoEquipos(true, item));
    }
    const [contador, setContador] = useState(0);
    const [miPagina, setmiPagina] = useState(1);
    const {registros, paginas} = useSelector(state => state.tipo_equipo)
    ...
    const handleEliminar = (item) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
               ...
               dispatch(eliminarTipoEquipo(obj));
            }
        })
    }
    const handlePaginar = (index, pagina) => {
        ...
        if (valor >= 0 && valor <= registros) {
            console.log(valor)
            dispatch(getTiposEquiposAdmin(valor));
