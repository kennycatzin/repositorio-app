...
import { eliminarLicencia, getLicenciasAdmin, openCloseModalLicencias } from '../../../../actions/licencia';
import { LicenciaModal } from '../modal/LicenciaModal';

export const TableLicencia = ({ cabeceras, data }) => {
    const dispatch = useDispatch();
    const handleEditar = (item) => {
        dispatch(openCloseModalLicencias(true, item));
    }
    const [contador, setContador] = useState(0);
    const [miPagina, setmiPagina] = useState(1);
    const {registros, paginas} = useSelector(state => state.licencia)
                ...
                <tbody>
                    {
                        data.map((item, index) => {
                            return <tr key={item.id}>
                                ...
                                <td className='text-center'>{item.licencia}</td>
